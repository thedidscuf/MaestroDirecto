...
CREATE TABLE IF NOT EXISTS password_reset_logs (
  ...
  status text NOT NULL DEFAULT 'requested' CHECK (status IN ('requested', 'sent', 'used', 'expired', 'failed', 'rate_limited', 'user_not_found', 'error', 'system_init')),
  ip_address inet,
  user_agent text,
...
CREATE INDEX IF NOT EXISTS idx_password_reset_logs_expires ON password_reset_logs(expires_at);
...
BEGIN
  ...
  WHERE 
    password_reset_logs.email_address = check_password_reset_rate_limit.email_address
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM password_reset_logs WHERE email_address = 'system@maestrodirecto.cl') THEN
    INSERT INTO password_reset_logs (user_id, email_address, status, created_at)
    VALUES (NULL, 'system@maestrodirecto.cl', 'system_init', now());
  END IF;
END $$;

-- Ejecutar limpieza inicial usando DO block
DO $$
BEGIN
  PERFORM cleanup_old_password_reset_logs();
END $$;